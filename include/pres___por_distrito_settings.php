...
if(mlang_getcurrentlang()=="Spanish")
{
	...
	$fieldLabelspres___por_distrito["Spanish"]["Otros"] = "Otros";
	$fieldToolTipspres___por_distrito["Spanish"]["Otros"] = "";
	$placeHolderspres___por_distrito["Spanish"]["Otros"] = "";
...
$tdatapres___por_distrito[".googleLikeFields"][] = "Otros";
...
$tdatapres___por_distrito[".sqlHead"] = "select ur.id_distrito,   (select concat(sum(p.candidato1), \" votos (\", ROUND(((sum(p.candidato1))/(sum(validos))*100),1), \"%)\")    from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as PAC,  (select concat(sum(p.candidato2), \" votos (\", ROUND((((sum(p.candidato2)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as FAPER,  (select concat(sum(p.candidato3), \" votos (\", ROUND((((sum(p.candidato3)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as NUEVA_RUTA,  (select concat(sum(p.candidato4), \" votos (\", ROUND((((sum(p.candidato4)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as ALIANZA_PAT,  (select concat(sum(p.candidato5), \" votos (\", ROUND((((sum(p.candidato5)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as MIDE,  (select concat(sum(p.candidato6), \" votos (\", ROUND((((sum(p.candidato6)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as VAMOS,  (select concat(sum(p.candidato7), \" votos (\", ROUND((((sum(p.candidato7)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as LIBRE,  (select concat(sum(p.candidato8), \" votos (\", ROUND((((sum(p.candidato8)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as PSH,  (select concat(sum(p.candidato9), \" votos (\", ROUND((((sum(p.candidato9)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as LIDEHR,  (select concat(sum(p.candidato10), \" votos (\", ROUND((((sum(p.candidato10)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as UD,  (select concat(sum(p.candidato11), \" votos (\", ROUND((((sum(p.candidato11)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as DC,  (select concat(sum(p.candidato12), \" votos (\", ROUND((((sum(p.candidato12)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as TSH,  (select concat(sum(p.candidato13), \" votos (\", ROUND((((sum(p.candidato13)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as HON_HUMANA,  (select concat(sum(p.candidato14), \" votos (\", ROUND((((sum(p.candidato14)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as LIBERAL,  (select concat(sum(p.candidato15), \" votos (\", ROUND((((sum(p.candidato15)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as NACIONAL,  (select concat(((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))), \" votos (\", ROUND((((((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))) / (sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Otros,  (select sum(p.validos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Validos,  (select sum(p.blancos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Blancos,  (select sum(p.nulos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Nulos,  (select sum(p.utilizados) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Utilizados,  (select sum(p.no_utilizados) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as No_Utilizados,  (select sum(p.total) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Total";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato1), \" votos (\", ROUND(((sum(p.candidato1))/(sum(validos))*100),1), \"%)\")    from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato2), \" votos (\", ROUND((((sum(p.candidato2)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato3), \" votos (\", ROUND((((sum(p.candidato3)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato4), \" votos (\", ROUND((((sum(p.candidato4)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato5), \" votos (\", ROUND((((sum(p.candidato5)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato6), \" votos (\", ROUND((((sum(p.candidato6)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato7), \" votos (\", ROUND((((sum(p.candidato7)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato8), \" votos (\", ROUND((((sum(p.candidato8)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato9), \" votos (\", ROUND((((sum(p.candidato9)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato10), \" votos (\", ROUND((((sum(p.candidato10)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato11), \" votos (\", ROUND((((sum(p.candidato11)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato12), \" votos (\", ROUND((((sum(p.candidato12)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato13), \" votos (\", ROUND((((sum(p.candidato13)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato14), \" votos (\", ROUND((((sum(p.candidato14)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	// end of report field settings
		...
		$fdata["isSQLExpression"] = true;
	$fdata["FullName"] = "(select concat(sum(p.candidato15), \" votos (\", ROUND((((sum(p.candidato15)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
//	Custom field settings
	...
	$fdata["ownerTable"] = "";
	$fdata["Label"] = GetFieldLabel("Pres___Por_Distrito","Otros");
	$fdata["FieldType"] = 200;
...
//end of Filters settings
	...
	$tdatapres___por_distrito["Otros"] = $fdata;
		$tdatapres___por_distrito[".searchableFields"][] = "Otros";
//	Validos
//	Custom field settings
	$fdata = array();
	$fdata["Index"] = 18;
...
$proto0["m_strFieldList"] = "ur.id_distrito,   (select concat(sum(p.candidato1), \" votos (\", ROUND(((sum(p.candidato1))/(sum(validos))*100),1), \"%)\")    from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as PAC,  (select concat(sum(p.candidato2), \" votos (\", ROUND((((sum(p.candidato2)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as FAPER,  (select concat(sum(p.candidato3), \" votos (\", ROUND((((sum(p.candidato3)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as NUEVA_RUTA,  (select concat(sum(p.candidato4), \" votos (\", ROUND((((sum(p.candidato4)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as ALIANZA_PAT,  (select concat(sum(p.candidato5), \" votos (\", ROUND((((sum(p.candidato5)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as MIDE,  (select concat(sum(p.candidato6), \" votos (\", ROUND((((sum(p.candidato6)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as VAMOS,  (select concat(sum(p.candidato7), \" votos (\", ROUND((((sum(p.candidato7)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as LIBRE,  (select concat(sum(p.candidato8), \" votos (\", ROUND((((sum(p.candidato8)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as PSH,  (select concat(sum(p.candidato9), \" votos (\", ROUND((((sum(p.candidato9)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as LIDEHR,  (select concat(sum(p.candidato10), \" votos (\", ROUND((((sum(p.candidato10)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as UD,  (select concat(sum(p.candidato11), \" votos (\", ROUND((((sum(p.candidato11)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as DC,  (select concat(sum(p.candidato12), \" votos (\", ROUND((((sum(p.candidato12)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as TSH,  (select concat(sum(p.candidato13), \" votos (\", ROUND((((sum(p.candidato13)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as HON_HUMANA,  (select concat(sum(p.candidato14), \" votos (\", ROUND((((sum(p.candidato14)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as LIBERAL,  (select concat(sum(p.candidato15), \" votos (\", ROUND((((sum(p.candidato15)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as NACIONAL,  (select concat(((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))), \" votos (\", ROUND((((((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))) / (sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Otros,  (select sum(p.validos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Validos,  (select sum(p.blancos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Blancos,  (select sum(p.nulos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Nulos,  (select sum(p.utilizados) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Utilizados,  (select sum(p.no_utilizados) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as No_Utilizados,  (select sum(p.total) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito) as Total";
...
$proto9["m_strFieldList"] = "concat(sum(p.candidato1), \" votos (\", ROUND(((sum(p.candidato1))/(sum(validos))*100),1), \"%)\")";
...
$proto15["m_sql"] = "concat(sum(p.candidato1), \" votos (\", ROUND(((sum(p.candidato1))/(sum(validos))*100),1), \"%)\")";
...
$proto8["m_sql"] = "(select concat(sum(p.candidato1), \" votos (\", ROUND(((sum(p.candidato1))/(sum(validos))*100),1), \"%)\")    from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto30["m_strFieldList"] = "concat(sum(p.candidato2), \" votos (\", ROUND((((sum(p.candidato2)/(sum(validos))))*100),1), \"%)\")";
...
$proto36["m_sql"] = "concat(sum(p.candidato2), \" votos (\", ROUND((((sum(p.candidato2)/(sum(validos))))*100),1), \"%)\")";
...
$proto29["m_sql"] = "(select concat(sum(p.candidato2), \" votos (\", ROUND((((sum(p.candidato2)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto51["m_strFieldList"] = "concat(sum(p.candidato3), \" votos (\", ROUND((((sum(p.candidato3)/(sum(validos))))*100),1), \"%)\")";
...
$proto57["m_sql"] = "concat(sum(p.candidato3), \" votos (\", ROUND((((sum(p.candidato3)/(sum(validos))))*100),1), \"%)\")";
...
$proto50["m_sql"] = "(select concat(sum(p.candidato3), \" votos (\", ROUND((((sum(p.candidato3)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto72["m_strFieldList"] = "concat(sum(p.candidato4), \" votos (\", ROUND((((sum(p.candidato4)/(sum(validos))))*100),1), \"%)\")";
...
$proto78["m_sql"] = "concat(sum(p.candidato4), \" votos (\", ROUND((((sum(p.candidato4)/(sum(validos))))*100),1), \"%)\")";
...
$proto71["m_sql"] = "(select concat(sum(p.candidato4), \" votos (\", ROUND((((sum(p.candidato4)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto93["m_strFieldList"] = "concat(sum(p.candidato5), \" votos (\", ROUND((((sum(p.candidato5)/(sum(validos))))*100),1), \"%)\")";
...
$proto99["m_sql"] = "concat(sum(p.candidato5), \" votos (\", ROUND((((sum(p.candidato5)/(sum(validos))))*100),1), \"%)\")";
...
$proto92["m_sql"] = "(select concat(sum(p.candidato5), \" votos (\", ROUND((((sum(p.candidato5)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto114["m_strFieldList"] = "concat(sum(p.candidato6), \" votos (\", ROUND((((sum(p.candidato6)/(sum(validos))))*100),1), \"%)\")";
...
$proto120["m_sql"] = "concat(sum(p.candidato6), \" votos (\", ROUND((((sum(p.candidato6)/(sum(validos))))*100),1), \"%)\")";
...
$proto113["m_sql"] = "(select concat(sum(p.candidato6), \" votos (\", ROUND((((sum(p.candidato6)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto135["m_strFieldList"] = "concat(sum(p.candidato7), \" votos (\", ROUND((((sum(p.candidato7)/(sum(validos))))*100),1), \"%)\")";
...
$proto141["m_sql"] = "concat(sum(p.candidato7), \" votos (\", ROUND((((sum(p.candidato7)/(sum(validos))))*100),1), \"%)\")";
...
$proto134["m_sql"] = "(select concat(sum(p.candidato7), \" votos (\", ROUND((((sum(p.candidato7)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto156["m_strFieldList"] = "concat(sum(p.candidato8), \" votos (\", ROUND((((sum(p.candidato8)/(sum(validos))))*100),1), \"%)\")";
...
$proto162["m_sql"] = "concat(sum(p.candidato8), \" votos (\", ROUND((((sum(p.candidato8)/(sum(validos))))*100),1), \"%)\")";
...
$proto155["m_sql"] = "(select concat(sum(p.candidato8), \" votos (\", ROUND((((sum(p.candidato8)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto177["m_strFieldList"] = "concat(sum(p.candidato9), \" votos (\", ROUND((((sum(p.candidato9)/(sum(validos))))*100),1), \"%)\")";
...
$proto183["m_sql"] = "concat(sum(p.candidato9), \" votos (\", ROUND((((sum(p.candidato9)/(sum(validos))))*100),1), \"%)\")";
...
$proto176["m_sql"] = "(select concat(sum(p.candidato9), \" votos (\", ROUND((((sum(p.candidato9)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto198["m_strFieldList"] = "concat(sum(p.candidato10), \" votos (\", ROUND((((sum(p.candidato10)/(sum(validos))))*100),1), \"%)\")";
...
$proto204["m_sql"] = "concat(sum(p.candidato10), \" votos (\", ROUND((((sum(p.candidato10)/(sum(validos))))*100),1), \"%)\")";
...
$proto197["m_sql"] = "(select concat(sum(p.candidato10), \" votos (\", ROUND((((sum(p.candidato10)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto219["m_strFieldList"] = "concat(sum(p.candidato11), \" votos (\", ROUND((((sum(p.candidato11)/(sum(validos))))*100),1), \"%)\")";
...
$proto225["m_sql"] = "concat(sum(p.candidato11), \" votos (\", ROUND((((sum(p.candidato11)/(sum(validos))))*100),1), \"%)\")";
...
$proto218["m_sql"] = "(select concat(sum(p.candidato11), \" votos (\", ROUND((((sum(p.candidato11)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto240["m_strFieldList"] = "concat(sum(p.candidato12), \" votos (\", ROUND((((sum(p.candidato12)/(sum(validos))))*100),1), \"%)\")";
...
$proto246["m_sql"] = "concat(sum(p.candidato12), \" votos (\", ROUND((((sum(p.candidato12)/(sum(validos))))*100),1), \"%)\")";
...
$proto239["m_sql"] = "(select concat(sum(p.candidato12), \" votos (\", ROUND((((sum(p.candidato12)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto261["m_strFieldList"] = "concat(sum(p.candidato13), \" votos (\", ROUND((((sum(p.candidato13)/(sum(validos))))*100),1), \"%)\")";
...
$proto267["m_sql"] = "concat(sum(p.candidato13), \" votos (\", ROUND((((sum(p.candidato13)/(sum(validos))))*100),1), \"%)\")";
...
$proto260["m_sql"] = "(select concat(sum(p.candidato13), \" votos (\", ROUND((((sum(p.candidato13)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto282["m_strFieldList"] = "concat(sum(p.candidato14), \" votos (\", ROUND((((sum(p.candidato14)/(sum(validos))))*100),1), \"%)\")";
...
$proto288["m_sql"] = "concat(sum(p.candidato14), \" votos (\", ROUND((((sum(p.candidato14)/(sum(validos))))*100),1), \"%)\")";
...
$proto281["m_sql"] = "(select concat(sum(p.candidato14), \" votos (\", ROUND((((sum(p.candidato14)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto303["m_strFieldList"] = "concat(sum(p.candidato15), \" votos (\", ROUND((((sum(p.candidato15)/(sum(validos))))*100),1), \"%)\")";
...
$proto309["m_sql"] = "concat(sum(p.candidato15), \" votos (\", ROUND((((sum(p.candidato15)/(sum(validos))))*100),1), \"%)\")";
...
$proto302["m_sql"] = "(select concat(sum(p.candidato15), \" votos (\", ROUND((((sum(p.candidato15)/(sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
...
$proto324["m_strFieldList"] = "concat(((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))), \" votos (\", ROUND((((((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))) / (sum(validos))))*100),1), \"%)\")";
...
$proto324["m_fieldlist"] = array();
						$proto330=array();
			$proto331=array();
$proto331["m_functiontype"] = "SQLF_CUSTOM";
...
$proto331["m_arguments"][]=$obj;
$proto331["m_strFunctionName"] = "concat";
$obj = new SQLFunctionCall($proto331);

$proto330["m_sql"] = "concat(((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))), \" votos (\", ROUND((((((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))) / (sum(validos))))*100),1), \"%)\")";
...
$proto324["m_fromlist"] = array();
												$proto336=array();
$proto336["m_link"] = "SQLL_MAIN";
			$proto337=array();
$proto337["m_strName"] = "voto_presidente";
$proto337["m_srcTableName"] = "Pres - Por Distrito";
$proto337["m_columns"] = array();
$proto337["m_columns"][] = "elecciones";
$proto337["m_columns"][] = "id_urna";
$proto337["m_columns"][] = "candidato1";
$proto337["m_columns"][] = "candidato2";
$proto337["m_columns"][] = "candidato3";
$proto337["m_columns"][] = "candidato4";
$proto337["m_columns"][] = "candidato5";
$proto337["m_columns"][] = "candidato6";
$proto337["m_columns"][] = "candidato7";
$proto337["m_columns"][] = "candidato8";
$proto337["m_columns"][] = "candidato9";
$proto337["m_columns"][] = "candidato10";
$proto337["m_columns"][] = "candidato11";
$proto337["m_columns"][] = "candidato12";
$proto337["m_columns"][] = "candidato13";
$proto337["m_columns"][] = "candidato14";
$proto337["m_columns"][] = "candidato15";
$proto337["m_columns"][] = "validos";
$proto337["m_columns"][] = "blancos";
$proto337["m_columns"][] = "nulos";
$proto337["m_columns"][] = "utilizados";
$proto337["m_columns"][] = "no_utilizados";
$proto337["m_columns"][] = "total";
$proto337["m_columns"][] = "usuario";
$proto337["m_columns"][] = "fecha";
$proto337["m_columns"][] = "editado_por";
$proto337["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto337);

$proto336["m_table"] = $obj;
$proto336["m_sql"] = "voto_presidente as p";
$proto336["m_alias"] = "p";
$proto336["m_srcTableName"] = "Pres - Por Distrito";
$proto338=array();
$proto338["m_sql"] = "";
...
$proto336["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto336);

$proto324["m_fromlist"][]=$obj;
												$proto340=array();
$proto340["m_link"] = "SQLL_INNERJOIN";
			$proto341=array();
$proto341["m_strName"] = "urna";
$proto341["m_srcTableName"] = "Pres - Por Distrito";
$proto341["m_columns"] = array();
$proto341["m_columns"][] = "id_distrito";
$proto341["m_columns"][] = "id_sector";
$proto341["m_columns"][] = "id_urna";
$proto341["m_columns"][] = "nombre_centro";
$proto341["m_columns"][] = "ubicación";
$proto341["m_columns"][] = "votantes";
$obj = new SQLTable($proto341);

$proto340["m_table"] = $obj;
$proto340["m_sql"] = "inner join urna as u on p.id_urna = u.id_urna";
$proto340["m_alias"] = "u";
$proto340["m_srcTableName"] = "Pres - Por Distrito";
$proto342=array();
$proto342["m_sql"] = "p.id_urna = u.id_urna";
...
$proto323["m_sql"] = "(select concat(((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))), \" votos (\", ROUND((((((sum(p.candidato1)) + (sum(p.candidato2)) + (sum(p.candidato3)) + (sum(p.candidato4)) + (sum(p.candidato5)) + (sum(p.candidato6)) + (sum(p.candidato8))   + (sum(p.candidato9)) + (sum(p.candidato10)) + (sum(p.candidato11)) + (sum(p.candidato12)) + (sum(p.candidato13))) / (sum(validos))))*100),1), \"%)\")  from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
$proto323["m_srcTableName"] = "Pres - Por Distrito";
$proto323["m_expr"]=$obj;
$proto323["m_alias"] = "Otros";
$obj = new SQLFieldListItem($proto323);

$proto0["m_fieldlist"][]=$obj;
						$proto344=array();
			$proto345=array();
$proto345["m_strHead"] = "select";
$proto345["m_strFieldList"] = "sum(p.validos)";
$proto345["m_strFrom"] = "from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna";
$proto345["m_strWhere"] = "u.id_distrito = ur.id_distrito";
$proto345["m_strOrderBy"] = "";
	
		;
			$proto345["cipherer"] = null;
$proto347=array();
$proto347["m_sql"] = "u.id_distrito = ur.id_distrito";
...
$proto345["m_having"] = $obj;
$proto345["m_fieldlist"] = array();
						$proto351=array();
			$proto352=array();
$proto352["m_functiontype"] = "SQLF_SUM";
...
$proto352["m_strFunctionName"] = "sum";
$obj = new SQLFunctionCall($proto352);

$proto351["m_sql"] = "sum(p.validos)";
$proto351["m_srcTableName"] = "Pres - Por Distrito";
$proto351["m_expr"]=$obj;
$proto351["m_alias"] = "";
$obj = new SQLFieldListItem($proto351);

$proto345["m_fieldlist"][]=$obj;
$proto345["m_fromlist"] = array();
												$proto354=array();
$proto354["m_link"] = "SQLL_MAIN";
			$proto355=array();
$proto355["m_strName"] = "voto_presidente";
$proto355["m_srcTableName"] = "Pres - Por Distrito";
$proto355["m_columns"] = array();
$proto355["m_columns"][] = "elecciones";
$proto355["m_columns"][] = "id_urna";
$proto355["m_columns"][] = "candidato1";
$proto355["m_columns"][] = "candidato2";
$proto355["m_columns"][] = "candidato3";
$proto355["m_columns"][] = "candidato4";
$proto355["m_columns"][] = "candidato5";
$proto355["m_columns"][] = "candidato6";
$proto355["m_columns"][] = "candidato7";
$proto355["m_columns"][] = "candidato8";
$proto355["m_columns"][] = "candidato9";
$proto355["m_columns"][] = "candidato10";
$proto355["m_columns"][] = "candidato11";
$proto355["m_columns"][] = "candidato12";
$proto355["m_columns"][] = "candidato13";
$proto355["m_columns"][] = "candidato14";
$proto355["m_columns"][] = "candidato15";
$proto355["m_columns"][] = "validos";
$proto355["m_columns"][] = "blancos";
$proto355["m_columns"][] = "nulos";
$proto355["m_columns"][] = "utilizados";
$proto355["m_columns"][] = "no_utilizados";
$proto355["m_columns"][] = "total";
$proto355["m_columns"][] = "usuario";
$proto355["m_columns"][] = "fecha";
$proto355["m_columns"][] = "editado_por";
$proto355["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto355);

$proto354["m_table"] = $obj;
$proto354["m_sql"] = "voto_presidente as p";
$proto354["m_alias"] = "p";
$proto354["m_srcTableName"] = "Pres - Por Distrito";
$proto356=array();
$proto356["m_sql"] = "";
...
$proto345["m_fromlist"][]=$obj;
												$proto358=array();
$proto358["m_link"] = "SQLL_INNERJOIN";
			$proto359=array();
$proto359["m_strName"] = "urna";
$proto359["m_srcTableName"] = "Pres - Por Distrito";
$proto359["m_columns"] = array();
$proto359["m_columns"][] = "id_distrito";
$proto359["m_columns"][] = "id_sector";
$proto359["m_columns"][] = "id_urna";
$proto359["m_columns"][] = "nombre_centro";
$proto359["m_columns"][] = "ubicación";
$proto359["m_columns"][] = "votantes";
$obj = new SQLTable($proto359);

$proto358["m_table"] = $obj;
$proto358["m_sql"] = "inner join urna as u on p.id_urna = u.id_urna";
$proto358["m_alias"] = "u";
$proto358["m_srcTableName"] = "Pres - Por Distrito";
$proto360=array();
$proto360["m_sql"] = "p.id_urna = u.id_urna";
...
$proto345["m_fromlist"][]=$obj;
$proto345["m_groupby"] = array();
$proto345["m_orderby"] = array();
$proto345["m_srcTableName"]="Pres - Por Distrito";		
$obj = new SQLQuery($proto345);

$proto344["m_sql"] = "(select sum(p.validos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
$proto344["m_srcTableName"] = "Pres - Por Distrito";
$proto344["m_expr"]=$obj;
$proto344["m_alias"] = "Validos";
$obj = new SQLFieldListItem($proto344);

$proto0["m_fieldlist"][]=$obj;
						$proto362=array();
			$proto363=array();
$proto363["m_strHead"] = "select";
$proto363["m_strFieldList"] = "sum(p.blancos)";
$proto363["m_strFrom"] = "from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna";
$proto363["m_strWhere"] = "u.id_distrito = ur.id_distrito";
$proto363["m_strOrderBy"] = "";
	
		;
			$proto363["cipherer"] = null;
$proto365=array();
$proto365["m_sql"] = "u.id_distrito = ur.id_distrito";
...
$proto363["m_having"] = $obj;
$proto363["m_fieldlist"] = array();
						$proto369=array();
			$proto370=array();
$proto370["m_functiontype"] = "SQLF_SUM";
...
$proto370["m_strFunctionName"] = "sum";
$obj = new SQLFunctionCall($proto370);

$proto369["m_sql"] = "sum(p.blancos)";
$proto369["m_srcTableName"] = "Pres - Por Distrito";
$proto369["m_expr"]=$obj;
$proto369["m_alias"] = "";
$obj = new SQLFieldListItem($proto369);

$proto363["m_fieldlist"][]=$obj;
$proto363["m_fromlist"] = array();
												$proto372=array();
$proto372["m_link"] = "SQLL_MAIN";
			$proto373=array();
$proto373["m_strName"] = "voto_presidente";
$proto373["m_srcTableName"] = "Pres - Por Distrito";
$proto373["m_columns"] = array();
$proto373["m_columns"][] = "elecciones";
$proto373["m_columns"][] = "id_urna";
$proto373["m_columns"][] = "candidato1";
$proto373["m_columns"][] = "candidato2";
$proto373["m_columns"][] = "candidato3";
$proto373["m_columns"][] = "candidato4";
$proto373["m_columns"][] = "candidato5";
$proto373["m_columns"][] = "candidato6";
$proto373["m_columns"][] = "candidato7";
$proto373["m_columns"][] = "candidato8";
$proto373["m_columns"][] = "candidato9";
$proto373["m_columns"][] = "candidato10";
$proto373["m_columns"][] = "candidato11";
$proto373["m_columns"][] = "candidato12";
$proto373["m_columns"][] = "candidato13";
$proto373["m_columns"][] = "candidato14";
$proto373["m_columns"][] = "candidato15";
$proto373["m_columns"][] = "validos";
$proto373["m_columns"][] = "blancos";
$proto373["m_columns"][] = "nulos";
$proto373["m_columns"][] = "utilizados";
$proto373["m_columns"][] = "no_utilizados";
$proto373["m_columns"][] = "total";
$proto373["m_columns"][] = "usuario";
$proto373["m_columns"][] = "fecha";
$proto373["m_columns"][] = "editado_por";
$proto373["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto373);

$proto372["m_table"] = $obj;
$proto372["m_sql"] = "voto_presidente as p";
$proto372["m_alias"] = "p";
$proto372["m_srcTableName"] = "Pres - Por Distrito";
$proto374=array();
$proto374["m_sql"] = "";
...
$proto372["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto372);

$proto363["m_fromlist"][]=$obj;
												$proto376=array();
$proto376["m_link"] = "SQLL_INNERJOIN";
			$proto377=array();
$proto377["m_strName"] = "urna";
$proto377["m_srcTableName"] = "Pres - Por Distrito";
$proto377["m_columns"] = array();
$proto377["m_columns"][] = "id_distrito";
$proto377["m_columns"][] = "id_sector";
$proto377["m_columns"][] = "id_urna";
$proto377["m_columns"][] = "nombre_centro";
$proto377["m_columns"][] = "ubicación";
$proto377["m_columns"][] = "votantes";
$obj = new SQLTable($proto377);

$proto376["m_table"] = $obj;
$proto376["m_sql"] = "inner join urna as u on p.id_urna = u.id_urna";
$proto376["m_alias"] = "u";
$proto376["m_srcTableName"] = "Pres - Por Distrito";
$proto378=array();
$proto378["m_sql"] = "p.id_urna = u.id_urna";
...
$proto378["m_strCase"] = "= u.id_urna";
$proto378["m_havingmode"] = false;
$proto378["m_inBrackets"] = false;
$proto378["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto378);

$proto376["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto376);

$proto363["m_fromlist"][]=$obj;
$proto363["m_groupby"] = array();
$proto363["m_orderby"] = array();
$proto363["m_srcTableName"]="Pres - Por Distrito";		
$obj = new SQLQuery($proto363);

$proto362["m_sql"] = "(select sum(p.blancos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
$proto362["m_srcTableName"] = "Pres - Por Distrito";
$proto362["m_expr"]=$obj;
$proto362["m_alias"] = "Blancos";
$obj = new SQLFieldListItem($proto362);

$proto0["m_fieldlist"][]=$obj;
						$proto380=array();
			$proto381=array();
$proto381["m_strHead"] = "select";
$proto381["m_strFieldList"] = "sum(p.nulos)";
$proto381["m_strFrom"] = "from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna";
$proto381["m_strWhere"] = "u.id_distrito = ur.id_distrito";
$proto381["m_strOrderBy"] = "";
	
		;
			$proto381["cipherer"] = null;
$proto383=array();
$proto383["m_sql"] = "u.id_distrito = ur.id_distrito";
...
$proto381["m_having"] = $obj;
$proto381["m_fieldlist"] = array();
						$proto387=array();
			$proto388=array();
$proto388["m_functiontype"] = "SQLF_SUM";
$proto388["m_arguments"] = array();
						$obj = new SQLField(array(
	"m_strName" => "nulos",
	"m_strTable" => "p",
	"m_srcTableName" => "Pres - Por Distrito"
...
$proto388["m_arguments"][]=$obj;
$proto388["m_strFunctionName"] = "sum";
$obj = new SQLFunctionCall($proto388);

$proto387["m_sql"] = "sum(p.nulos)";
$proto387["m_srcTableName"] = "Pres - Por Distrito";
$proto387["m_expr"]=$obj;
$proto387["m_alias"] = "";
$obj = new SQLFieldListItem($proto387);

$proto381["m_fieldlist"][]=$obj;
$proto381["m_fromlist"] = array();
												$proto390=array();
$proto390["m_link"] = "SQLL_MAIN";
			$proto391=array();
$proto391["m_strName"] = "voto_presidente";
$proto391["m_srcTableName"] = "Pres - Por Distrito";
$proto391["m_columns"] = array();
$proto391["m_columns"][] = "elecciones";
$proto391["m_columns"][] = "id_urna";
$proto391["m_columns"][] = "candidato1";
$proto391["m_columns"][] = "candidato2";
$proto391["m_columns"][] = "candidato3";
$proto391["m_columns"][] = "candidato4";
$proto391["m_columns"][] = "candidato5";
$proto391["m_columns"][] = "candidato6";
$proto391["m_columns"][] = "candidato7";
$proto391["m_columns"][] = "candidato8";
$proto391["m_columns"][] = "candidato9";
$proto391["m_columns"][] = "candidato10";
$proto391["m_columns"][] = "candidato11";
$proto391["m_columns"][] = "candidato12";
$proto391["m_columns"][] = "candidato13";
$proto391["m_columns"][] = "candidato14";
$proto391["m_columns"][] = "candidato15";
$proto391["m_columns"][] = "validos";
$proto391["m_columns"][] = "blancos";
$proto391["m_columns"][] = "nulos";
$proto391["m_columns"][] = "utilizados";
$proto391["m_columns"][] = "no_utilizados";
$proto391["m_columns"][] = "total";
$proto391["m_columns"][] = "usuario";
$proto391["m_columns"][] = "fecha";
$proto391["m_columns"][] = "editado_por";
$proto391["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto391);

$proto390["m_table"] = $obj;
$proto390["m_sql"] = "voto_presidente as p";
$proto390["m_alias"] = "p";
$proto390["m_srcTableName"] = "Pres - Por Distrito";
$proto392=array();
$proto392["m_sql"] = "";
$proto392["m_uniontype"] = "SQLL_UNKNOWN";
	$obj = new SQLNonParsed(array(
	"m_sql" => ""
));

$proto392["m_column"]=$obj;
$proto392["m_contained"] = array();
$proto392["m_strCase"] = "";
$proto392["m_havingmode"] = false;
$proto392["m_inBrackets"] = false;
$proto392["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto392);

$proto390["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto390);

$proto381["m_fromlist"][]=$obj;
												$proto394=array();
$proto394["m_link"] = "SQLL_INNERJOIN";
			$proto395=array();
$proto395["m_strName"] = "urna";
$proto395["m_srcTableName"] = "Pres - Por Distrito";
$proto395["m_columns"] = array();
$proto395["m_columns"][] = "id_distrito";
$proto395["m_columns"][] = "id_sector";
$proto395["m_columns"][] = "id_urna";
$proto395["m_columns"][] = "nombre_centro";
$proto395["m_columns"][] = "ubicación";
$proto395["m_columns"][] = "votantes";
$obj = new SQLTable($proto395);

$proto394["m_table"] = $obj;
$proto394["m_sql"] = "inner join urna as u on p.id_urna = u.id_urna";
$proto394["m_alias"] = "u";
$proto394["m_srcTableName"] = "Pres - Por Distrito";
$proto396=array();
$proto396["m_sql"] = "p.id_urna = u.id_urna";
$proto396["m_uniontype"] = "SQLL_UNKNOWN";
						$obj = new SQLField(array(
	"m_strName" => "id_urna",
	"m_strTable" => "p",
	"m_srcTableName" => "Pres - Por Distrito"
...
$proto396["m_column"]=$obj;
$proto396["m_contained"] = array();
$proto396["m_strCase"] = "= u.id_urna";
$proto396["m_havingmode"] = false;
$proto396["m_inBrackets"] = false;
$proto396["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto396);

$proto394["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto394);

$proto381["m_fromlist"][]=$obj;
$proto381["m_groupby"] = array();
$proto381["m_orderby"] = array();
$proto381["m_srcTableName"]="Pres - Por Distrito";		
$obj = new SQLQuery($proto381);

$proto380["m_sql"] = "(select sum(p.nulos) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
$proto380["m_srcTableName"] = "Pres - Por Distrito";
$proto380["m_expr"]=$obj;
$proto380["m_alias"] = "Nulos";
$obj = new SQLFieldListItem($proto380);

$proto0["m_fieldlist"][]=$obj;
						$proto398=array();
			$proto399=array();
$proto399["m_strHead"] = "select";
$proto399["m_strFieldList"] = "sum(p.utilizados)";
$proto399["m_strFrom"] = "from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna";
$proto399["m_strWhere"] = "u.id_distrito = ur.id_distrito";
$proto399["m_strOrderBy"] = "";
	
		;
			$proto399["cipherer"] = null;
$proto401=array();
$proto401["m_sql"] = "u.id_distrito = ur.id_distrito";
...
$proto399["m_having"] = $obj;
$proto399["m_fieldlist"] = array();
						$proto405=array();
			$proto406=array();
$proto406["m_functiontype"] = "SQLF_SUM";
...
$proto406["m_strFunctionName"] = "sum";
$obj = new SQLFunctionCall($proto406);

$proto405["m_sql"] = "sum(p.utilizados)";
$proto405["m_srcTableName"] = "Pres - Por Distrito";
$proto405["m_expr"]=$obj;
$proto405["m_alias"] = "";
$obj = new SQLFieldListItem($proto405);

$proto399["m_fieldlist"][]=$obj;
$proto399["m_fromlist"] = array();
												$proto408=array();
$proto408["m_link"] = "SQLL_MAIN";
			$proto409=array();
$proto409["m_strName"] = "voto_presidente";
$proto409["m_srcTableName"] = "Pres - Por Distrito";
$proto409["m_columns"] = array();
$proto409["m_columns"][] = "elecciones";
$proto409["m_columns"][] = "id_urna";
$proto409["m_columns"][] = "candidato1";
$proto409["m_columns"][] = "candidato2";
$proto409["m_columns"][] = "candidato3";
$proto409["m_columns"][] = "candidato4";
$proto409["m_columns"][] = "candidato5";
$proto409["m_columns"][] = "candidato6";
$proto409["m_columns"][] = "candidato7";
$proto409["m_columns"][] = "candidato8";
$proto409["m_columns"][] = "candidato9";
$proto409["m_columns"][] = "candidato10";
$proto409["m_columns"][] = "candidato11";
$proto409["m_columns"][] = "candidato12";
$proto409["m_columns"][] = "candidato13";
$proto409["m_columns"][] = "candidato14";
$proto409["m_columns"][] = "candidato15";
$proto409["m_columns"][] = "validos";
$proto409["m_columns"][] = "blancos";
$proto409["m_columns"][] = "nulos";
$proto409["m_columns"][] = "utilizados";
$proto409["m_columns"][] = "no_utilizados";
$proto409["m_columns"][] = "total";
$proto409["m_columns"][] = "usuario";
$proto409["m_columns"][] = "fecha";
$proto409["m_columns"][] = "editado_por";
$proto409["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto409);

$proto408["m_table"] = $obj;
$proto408["m_sql"] = "voto_presidente as p";
$proto408["m_alias"] = "p";
$proto408["m_srcTableName"] = "Pres - Por Distrito";
$proto410=array();
$proto410["m_sql"] = "";
$proto410["m_uniontype"] = "SQLL_UNKNOWN";
	$obj = new SQLNonParsed(array(
	"m_sql" => ""
));

$proto410["m_column"]=$obj;
$proto410["m_contained"] = array();
$proto410["m_strCase"] = "";
$proto410["m_havingmode"] = false;
$proto410["m_inBrackets"] = false;
$proto410["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto410);

$proto408["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto408);

$proto399["m_fromlist"][]=$obj;
												$proto412=array();
$proto412["m_link"] = "SQLL_INNERJOIN";
			$proto413=array();
$proto413["m_strName"] = "urna";
$proto413["m_srcTableName"] = "Pres - Por Distrito";
$proto413["m_columns"] = array();
$proto413["m_columns"][] = "id_distrito";
$proto413["m_columns"][] = "id_sector";
$proto413["m_columns"][] = "id_urna";
$proto413["m_columns"][] = "nombre_centro";
$proto413["m_columns"][] = "ubicación";
$proto413["m_columns"][] = "votantes";
$obj = new SQLTable($proto413);

$proto412["m_table"] = $obj;
$proto412["m_sql"] = "inner join urna as u on p.id_urna = u.id_urna";
$proto412["m_alias"] = "u";
$proto412["m_srcTableName"] = "Pres - Por Distrito";
$proto414=array();
$proto414["m_sql"] = "p.id_urna = u.id_urna";
...
$proto414["m_strCase"] = "= u.id_urna";
$proto414["m_havingmode"] = false;
$proto414["m_inBrackets"] = false;
$proto414["m_useAlias"] = false;
...
$proto399["m_fromlist"][]=$obj;
$proto399["m_groupby"] = array();
$proto399["m_orderby"] = array();
$proto399["m_srcTableName"]="Pres - Por Distrito";		
$obj = new SQLQuery($proto399);

$proto398["m_sql"] = "(select sum(p.utilizados) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
$proto398["m_srcTableName"] = "Pres - Por Distrito";
$proto398["m_expr"]=$obj;
$proto398["m_alias"] = "Utilizados";
...
$proto417["m_strFieldList"] = "sum(p.no_utilizados)";
$proto417["m_strFrom"] = "from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna";
$proto417["m_strWhere"] = "u.id_distrito = ur.id_distrito";
$proto417["m_strOrderBy"] = "";
	
		;
			$proto417["cipherer"] = null;
$proto419=array();
$proto419["m_sql"] = "u.id_distrito = ur.id_distrito";
$proto419["m_uniontype"] = "SQLL_UNKNOWN";
						$obj = new SQLField(array(
	"m_strName" => "id_distrito",
	"m_strTable" => "u",
	"m_srcTableName" => "Pres - Por Distrito"
...
$proto419["m_column"]=$obj;
$proto419["m_contained"] = array();
$proto419["m_strCase"] = "= ur.id_distrito";
$proto419["m_havingmode"] = false;
$proto419["m_inBrackets"] = false;
$proto419["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto419);
...
$proto417["m_fieldlist"] = array();
						$proto423=array();
			$proto424=array();
$proto424["m_functiontype"] = "SQLF_SUM";
$proto424["m_arguments"] = array();
						$obj = new SQLField(array(
...
$proto424["m_arguments"][]=$obj;
$proto424["m_strFunctionName"] = "sum";
$obj = new SQLFunctionCall($proto424);

$proto423["m_sql"] = "sum(p.no_utilizados)";
...
$proto417["m_fromlist"] = array();
												$proto426=array();
$proto426["m_link"] = "SQLL_MAIN";
			$proto427=array();
$proto427["m_strName"] = "voto_presidente";
$proto427["m_srcTableName"] = "Pres - Por Distrito";
$proto427["m_columns"] = array();
$proto427["m_columns"][] = "elecciones";
$proto427["m_columns"][] = "id_urna";
$proto427["m_columns"][] = "candidato1";
$proto427["m_columns"][] = "candidato2";
$proto427["m_columns"][] = "candidato3";
$proto427["m_columns"][] = "candidato4";
$proto427["m_columns"][] = "candidato5";
$proto427["m_columns"][] = "candidato6";
$proto427["m_columns"][] = "candidato7";
$proto427["m_columns"][] = "candidato8";
$proto427["m_columns"][] = "candidato9";
$proto427["m_columns"][] = "candidato10";
$proto427["m_columns"][] = "candidato11";
$proto427["m_columns"][] = "candidato12";
$proto427["m_columns"][] = "candidato13";
$proto427["m_columns"][] = "candidato14";
$proto427["m_columns"][] = "candidato15";
$proto427["m_columns"][] = "validos";
$proto427["m_columns"][] = "blancos";
$proto427["m_columns"][] = "nulos";
$proto427["m_columns"][] = "utilizados";
$proto427["m_columns"][] = "no_utilizados";
$proto427["m_columns"][] = "total";
$proto427["m_columns"][] = "usuario";
$proto427["m_columns"][] = "fecha";
$proto427["m_columns"][] = "editado_por";
$proto427["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto427);

$proto426["m_table"] = $obj;
$proto426["m_sql"] = "voto_presidente as p";
$proto426["m_alias"] = "p";
$proto426["m_srcTableName"] = "Pres - Por Distrito";
$proto428=array();
$proto428["m_sql"] = "";
...
$proto428["m_strCase"] = "";
$proto428["m_havingmode"] = false;
$proto428["m_inBrackets"] = false;
$proto428["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto428);

$proto426["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto426);

$proto417["m_fromlist"][]=$obj;
												$proto430=array();
$proto430["m_link"] = "SQLL_INNERJOIN";
			$proto431=array();
$proto431["m_strName"] = "urna";
$proto431["m_srcTableName"] = "Pres - Por Distrito";
$proto431["m_columns"] = array();
$proto431["m_columns"][] = "id_distrito";
$proto431["m_columns"][] = "id_sector";
$proto431["m_columns"][] = "id_urna";
$proto431["m_columns"][] = "nombre_centro";
$proto431["m_columns"][] = "ubicación";
$proto431["m_columns"][] = "votantes";
$obj = new SQLTable($proto431);

$proto430["m_table"] = $obj;
$proto430["m_sql"] = "inner join urna as u on p.id_urna = u.id_urna";
$proto430["m_alias"] = "u";
$proto430["m_srcTableName"] = "Pres - Por Distrito";
$proto432=array();
$proto432["m_sql"] = "p.id_urna = u.id_urna";
$proto432["m_uniontype"] = "SQLL_UNKNOWN";
						$obj = new SQLField(array(
	"m_strName" => "id_urna",
	"m_strTable" => "p",
	"m_srcTableName" => "Pres - Por Distrito"
...
$proto432["m_column"]=$obj;
$proto432["m_contained"] = array();
$proto432["m_strCase"] = "= u.id_urna";
$proto432["m_havingmode"] = false;
$proto432["m_inBrackets"] = false;
$proto432["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto432);

$proto430["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto430);

$proto417["m_fromlist"][]=$obj;
$proto417["m_groupby"] = array();
$proto417["m_orderby"] = array();
$proto417["m_srcTableName"]="Pres - Por Distrito";		
$obj = new SQLQuery($proto417);

$proto416["m_sql"] = "(select sum(p.no_utilizados) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
$proto416["m_srcTableName"] = "Pres - Por Distrito";
$proto416["m_expr"]=$obj;
$proto416["m_alias"] = "No_Utilizados";
$obj = new SQLFieldListItem($proto416);

$proto0["m_fieldlist"][]=$obj;
						$proto434=array();
			$proto435=array();
$proto435["m_strHead"] = "select";
$proto435["m_strFieldList"] = "sum(p.total)";
$proto435["m_strFrom"] = "from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna";
$proto435["m_strWhere"] = "u.id_distrito = ur.id_distrito";
$proto435["m_strOrderBy"] = "";
	
		;
			$proto435["cipherer"] = null;
$proto437=array();
$proto437["m_sql"] = "u.id_distrito = ur.id_distrito";
...
$proto435["m_having"] = $obj;
$proto435["m_fieldlist"] = array();
						$proto441=array();
			$proto442=array();
$proto442["m_functiontype"] = "SQLF_SUM";
...
$proto442["m_strFunctionName"] = "sum";
$obj = new SQLFunctionCall($proto442);

$proto441["m_sql"] = "sum(p.total)";
$proto441["m_srcTableName"] = "Pres - Por Distrito";
$proto441["m_expr"]=$obj;
$proto441["m_alias"] = "";
$obj = new SQLFieldListItem($proto441);

$proto435["m_fieldlist"][]=$obj;
$proto435["m_fromlist"] = array();
												$proto444=array();
$proto444["m_link"] = "SQLL_MAIN";
			$proto445=array();
$proto445["m_strName"] = "voto_presidente";
$proto445["m_srcTableName"] = "Pres - Por Distrito";
$proto445["m_columns"] = array();
$proto445["m_columns"][] = "elecciones";
$proto445["m_columns"][] = "id_urna";
$proto445["m_columns"][] = "candidato1";
$proto445["m_columns"][] = "candidato2";
$proto445["m_columns"][] = "candidato3";
$proto445["m_columns"][] = "candidato4";
$proto445["m_columns"][] = "candidato5";
$proto445["m_columns"][] = "candidato6";
$proto445["m_columns"][] = "candidato7";
$proto445["m_columns"][] = "candidato8";
$proto445["m_columns"][] = "candidato9";
$proto445["m_columns"][] = "candidato10";
$proto445["m_columns"][] = "candidato11";
$proto445["m_columns"][] = "candidato12";
$proto445["m_columns"][] = "candidato13";
$proto445["m_columns"][] = "candidato14";
$proto445["m_columns"][] = "candidato15";
$proto445["m_columns"][] = "validos";
$proto445["m_columns"][] = "blancos";
$proto445["m_columns"][] = "nulos";
$proto445["m_columns"][] = "utilizados";
$proto445["m_columns"][] = "no_utilizados";
$proto445["m_columns"][] = "total";
$proto445["m_columns"][] = "usuario";
$proto445["m_columns"][] = "fecha";
$proto445["m_columns"][] = "editado_por";
$proto445["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto445);

$proto444["m_table"] = $obj;
$proto444["m_sql"] = "voto_presidente as p";
$proto444["m_alias"] = "p";
$proto444["m_srcTableName"] = "Pres - Por Distrito";
$proto446=array();
$proto446["m_sql"] = "";
...
$proto446["m_strCase"] = "";
$proto446["m_havingmode"] = false;
$proto446["m_inBrackets"] = false;
$proto446["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto446);

$proto444["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto444);

$proto435["m_fromlist"][]=$obj;
												$proto448=array();
$proto448["m_link"] = "SQLL_INNERJOIN";
			$proto449=array();
$proto449["m_strName"] = "urna";
$proto449["m_srcTableName"] = "Pres - Por Distrito";
$proto449["m_columns"] = array();
$proto449["m_columns"][] = "id_distrito";
$proto449["m_columns"][] = "id_sector";
$proto449["m_columns"][] = "id_urna";
$proto449["m_columns"][] = "nombre_centro";
$proto449["m_columns"][] = "ubicación";
$proto449["m_columns"][] = "votantes";
$obj = new SQLTable($proto449);

$proto448["m_table"] = $obj;
$proto448["m_sql"] = "inner join urna as u on p.id_urna = u.id_urna";
$proto448["m_alias"] = "u";
$proto448["m_srcTableName"] = "Pres - Por Distrito";
$proto450=array();
$proto450["m_sql"] = "p.id_urna = u.id_urna";
$proto450["m_uniontype"] = "SQLL_UNKNOWN";
						$obj = new SQLField(array(
	"m_strName" => "id_urna",
	"m_strTable" => "p",
	"m_srcTableName" => "Pres - Por Distrito"
...
$proto450["m_contained"] = array();
$proto450["m_strCase"] = "= u.id_urna";
$proto450["m_havingmode"] = false;
$proto450["m_inBrackets"] = false;
$proto450["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto450);

$proto448["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto448);

$proto435["m_fromlist"][]=$obj;
$proto435["m_groupby"] = array();
$proto435["m_orderby"] = array();
$proto435["m_srcTableName"]="Pres - Por Distrito";		
$obj = new SQLQuery($proto435);

$proto434["m_sql"] = "(select sum(p.total) from voto_presidente as p  inner join urna as u on p.id_urna = u.id_urna where u.id_distrito = ur.id_distrito)";
$proto434["m_srcTableName"] = "Pres - Por Distrito";
$proto434["m_expr"]=$obj;
$proto434["m_alias"] = "Total";
...
$proto452["m_link"] = "SQLL_MAIN";
			$proto453=array();
$proto453["m_strName"] = "voto_presidente";
$proto453["m_srcTableName"] = "Pres - Por Distrito";
$proto453["m_columns"] = array();
$proto453["m_columns"][] = "elecciones";
$proto453["m_columns"][] = "id_urna";
$proto453["m_columns"][] = "candidato1";
$proto453["m_columns"][] = "candidato2";
$proto453["m_columns"][] = "candidato3";
$proto453["m_columns"][] = "candidato4";
$proto453["m_columns"][] = "candidato5";
$proto453["m_columns"][] = "candidato6";
$proto453["m_columns"][] = "candidato7";
$proto453["m_columns"][] = "candidato8";
$proto453["m_columns"][] = "candidato9";
$proto453["m_columns"][] = "candidato10";
$proto453["m_columns"][] = "candidato11";
$proto453["m_columns"][] = "candidato12";
$proto453["m_columns"][] = "candidato13";
$proto453["m_columns"][] = "candidato14";
$proto453["m_columns"][] = "candidato15";
$proto453["m_columns"][] = "validos";
$proto453["m_columns"][] = "blancos";
$proto453["m_columns"][] = "nulos";
$proto453["m_columns"][] = "utilizados";
$proto453["m_columns"][] = "no_utilizados";
$proto453["m_columns"][] = "total";
$proto453["m_columns"][] = "usuario";
$proto453["m_columns"][] = "fecha";
$proto453["m_columns"][] = "editado_por";
$proto453["m_columns"][] = "fecha_edicion";
$obj = new SQLTable($proto453);

$proto452["m_table"] = $obj;
$proto452["m_sql"] = "voto_presidente as vp";
$proto452["m_alias"] = "vp";
$proto452["m_srcTableName"] = "Pres - Por Distrito";
$proto454=array();
$proto454["m_sql"] = "";
...
$proto454["m_strCase"] = "";
$proto454["m_havingmode"] = false;
$proto454["m_inBrackets"] = false;
$proto454["m_useAlias"] = false;
...
$proto0["m_fromlist"][]=$obj;
												$proto456=array();
$proto456["m_link"] = "SQLL_INNERJOIN";
			$proto457=array();
$proto457["m_strName"] = "urna";
$proto457["m_srcTableName"] = "Pres - Por Distrito";
$proto457["m_columns"] = array();
$proto457["m_columns"][] = "id_distrito";
$proto457["m_columns"][] = "id_sector";
$proto457["m_columns"][] = "id_urna";
$proto457["m_columns"][] = "nombre_centro";
$proto457["m_columns"][] = "ubicación";
$proto457["m_columns"][] = "votantes";
$obj = new SQLTable($proto457);

$proto456["m_table"] = $obj;
$proto456["m_sql"] = "inner join urna as ur on vp.id_urna = ur.id_urna";
$proto456["m_alias"] = "ur";
$proto456["m_srcTableName"] = "Pres - Por Distrito";
$proto458=array();
$proto458["m_sql"] = "vp.id_urna = ur.id_urna";
$proto458["m_uniontype"] = "SQLL_UNKNOWN";
						$obj = new SQLField(array(
	"m_strName" => "id_urna",
	"m_strTable" => "vp",
	"m_srcTableName" => "Pres - Por Distrito"
...
$proto458["m_contained"] = array();
$proto458["m_strCase"] = "= ur.id_urna";
$proto458["m_havingmode"] = false;
$proto458["m_inBrackets"] = false;
$proto458["m_useAlias"] = false;
$obj = new SQLLogicalExpr($proto458);

$proto456["m_joinon"] = $obj;
$obj = new SQLFromListItem($proto456);
...
$obj = new SQLGroupByItem($proto460);
...
$proto462["m_bAsc"] = 1;
$proto462["m_nColumn"] = 0;
$obj = new SQLOrderByItem($proto462);
